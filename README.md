...
- **開始日**：2025年4月6日
- **現在の難易度**：★★★（中級・CSV編仕上げ段階）
  - `csv.reader()`・`csv.writer()`・`enumerate()`の実践型運用
  - 絶対パス指定の構え（`os.path.dirname(__file__)`）
  - ディレクトリ設計の最適化（`yyyy/mmdd/ファイル名` 構成）
  - VSCode実行時の動作やトラップも経験済み！

- **明日以降の予定**：
  - `csv.DictReader()` による列名アクセス型処理
  - 条件付き抽出＆構造的な書き出し
  - フォルダ構造とパスの自動生成術（必要なら）
