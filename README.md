...
- **開始日**：2025年4月6日
- **現在の難易度**：★★★★（CSV×インタラクション・設計思考編）
  - 複数ステップの処理（読み込み → 抽出 → 書き込み）における**ユーザー入力の分岐導入**
  - `"a"` or `"w"` モード選択に基づく書き込み処理の切り替えと構造整理
  - 処理の順序・設計・分離を見直すことで「ただ動くコード」から「目的に沿ったコード」への成長
  - `writerows()` との違い、入力バリデーション、if文構成の整え方にも気づきあり

- **次回の目標**：
  - 条件付きファイルマージ・ファイルごとのヘッダー処理
  - 入出力の例外処理設計
  - 関数化・再利用に向けた構え整理
